...
public class Operator extends User {
    ...
    try (Connection conn = config.DatabaseConfig.getConnection()) {
        ...
        try {
            String[] queries = {
                "DELETE FROM kel7.absensi WHERE id_pegawai = (SELECT id_pegawai FROM kel7.pegawai WHERE id_akun = ?)", // TAMBAHKAN INI
                "DELETE FROM kel7.laporan_kerja WHERE id_pegawai = (SELECT id_pegawai FROM kel7.pegawai WHERE id_akun = ?)",
                "DELETE FROM kel7.payroll WHERE id_pegawai = (SELECT id_pegawai FROM kel7.pegawai WHERE id_akun = ?)",
                "DELETE FROM kel7.cuti WHERE id_pegawai = (SELECT id_pegawai FROM kel7.pegawai WHERE id_akun = ?)",
                "DELETE FROM kel7.pegawai WHERE id_akun = ?",
                "DELETE FROM kel7.user_akun WHERE id_akun = ?"
            };
            ...
            conn.commit();
            System.out.println("[BERHASIL] Data telah dihapus sepenuhnya dari sistem.");
        } catch (SQLException e) {
            conn.rollback();
            System.out.println("[GAGAL] Terjadi kesalahan database: " + e.getMessage());
        }
        } catch (Exception e) { e.printStackTrace(); }
    }
}