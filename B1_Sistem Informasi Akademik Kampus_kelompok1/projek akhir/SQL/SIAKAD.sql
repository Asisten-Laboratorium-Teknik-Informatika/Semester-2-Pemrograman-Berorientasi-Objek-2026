...
-- PROGRAM STUDI
CREATE TYPE b1.jenjang_enum
AS ENUM('D3', 'D4', 'S1', 'S2', 'S3');

CREATE TYPE b1.akreditasi_enum 
...
CREATE TABLE b1.program_studi (
	...
	jenjang b1.jenjang_enum NOT NULL,
	akreditasi b1.akreditasi_enum NOT NULL
);
...
-- DOSEN
CREATE TYPE b1.jenis_kelamin_dosen_enum
...
CREATE TABLE b1.dosen (
	...
	jenis_kelamin b1.jenis_kelamin_dosen_enum NOT NULL,
	no_hp varchar (15),
...
-- MAHASISWA
CREATE TYPE b1.jenis_kelamin_mahasiswa_enum
AS ENUM('Laki-Laki', 'Perempuan');
...
CREATE TABLE b1.mahasiswa (
	...
	jenis_kelamin b1.jenis_kelamin_mahasiswa_enum NOT NULL,
...
-- AKUN
CREATE TYPE b1.role_akun_enum
AS ENUM('Mahasiswa', 'Dosen', 'Admin');
...
CREATE TABLE b1.akun (
	...
	role_akun b1.role_akun_enum NOT NULL,
...
-- TAHUN AKADEMIK
CREATE TYPE b1.semester_aktif_enum
AS ENUM('Ganjil', 'Genap');

CREATE TYPE b1.status_enum
AS ENUM('Aktif', 'Nonaktif');

CREATE TABLE b1.tahun_akademik (
	id_tahun varchar (10) PRIMARY KEY,
	tahun varchar (10) NOT NULL,
	semester_aktif b1.semester_aktif_enum NOT NULL,
	status b1.status_enum NOT NULL
);
...
-- KRS
CREATE TYPE b1.status_persetujuan_enum
...
CREATE TABLE b1.krs (
	...
	status_persetujuan b1.status_persetujuan_enum NOT NULL DEFAULT 'Menunggu',
	FOREIGN KEY (nim) REFERENCES b1.mahasiswa (nim),
	FOREIGN KEY (id_tahun) REFERENCES b1.tahun_akademik (id_tahun)
);
...
-- PRESENSI
CREATE TYPE b1.status_kehadiran_enum
...
CREATE TABLE b1.presensi (
	...
	status_kehadiran b1.status_kehadiran_enum NOT NULL DEFAULT 'Absen',
...
--DOSEN PEMBIMBING
CREATE TABLE b1.dosen_pembimbing (
    id_pembimbing  VARCHAR(10) PRIMARY KEY,
    nidn           VARCHAR(15) NOT NULL,
    nim            VARCHAR(20) NOT NULL,
    FOREIGN KEY (nidn)     REFERENCES b1.dosen(nidn),
    FOREIGN KEY (nim)      REFERENCES b1.mahasiswa(nim)
...
SELECT * FROM b1.dosen_pembimbing;

DROP TABLE b1.dosen_pembimbing;
...
LEFT JOIN b1.ruangan r   ON kk.id_ruangan = r.id_ruangan
...
ALTER TABLE b1.krs
ADD COLUMN nidn_pembimbing VARCHAR(15),
ADD FOREIGN KEY (nidn_pembimbing) REFERENCES b1.dosen(nidn);
